...
class Main(QWidget):
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_PageUp and int(self.map_api.zoom) < 17:
            ...
            self.map_api.draw()
            self.image.setPixmap(QPixmap('map.png'))
        elif event.key() == Qt.Key_Left and int(self.map_api.cords[0]) - 3 >= 0:
            self.map_api.cords[0] = str(int(self.map_api.cords[0]) - 3)
            self.map_api.draw()
            self.image.setPixmap(QPixmap('map.png'))
        elif event.key() == Qt.Key_Right and int(self.map_api.cords[0]) + 3 <= 180:
            self.map_api.cords[0] = str(int(self.map_api.cords[0]) + 3)
            self.map_api.draw()
            self.image.setPixmap(QPixmap('map.png'))
            ...
            self.map_api.draw()
            self.image.setPixmap(QPixmap('map.png'))
        elif event.key() == Qt.Key_Down and int(self.map_api.cords[1]) - 2 >= -90:
            self.map_api.cords[1] = str(int(self.map_api.cords[1]) - 2)
            self.map_api.draw()
            self.image.setPixmap(QPixmap('map.png'))
